...
| — | **Déconnexion** (menu ⋯) : oublie langue (cookie) + numéro (localStorage) → `chosen=false` → l'onboarding réapparaît (réinitialisé slide 0) ; `phone`/`logout` portés par `LocaleProvider` (badge d'accueil réactif). **Bannières de page** (`PageBanner`) en tête de `/conseils` et `/rumeurs` avec images dédiées (`public/conseils.png`, `public/rumeur.png`), remplaçant `PageIntro` sur ces deux pages | ✅ FAIT (demandé en cours) |
